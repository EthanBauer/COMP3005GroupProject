...
def Q_1(conn, execution_time):
    ...
    query = """SELECT player, AVG(xg_score) AS average_xg_score
FROM events_shot 
...
def Q_3(conn, execution_time):
    ...
    query = """SELECT player, COUNT(events.event_id) AS first_shot_count
FROM events_shot 
JOIN events ON events_shot.event_id = events.event_id
JOIN matches ON events.match_id = matches.match_id
...
def Q_4(conn, execution_time):
    ...
    query = """SELECT team, COUNT(*) AS num_passes
FROM events_pass
...
def Q_5(conn, execution_time):
    ...
    query = """SELECT recipient, COUNT(events_pass.event_id) AS intended_recipient_count
FROM events_pass
JOIN events ON events_pass.event_id = events.event_id
JOIN matches ON events.match_id = matches.match_id
WHERE matches.season_id = 44 AND matches.competition_id = 2 AND recipient_id IS NOT null 
...
def Q_6(conn, execution_time):
    ...
    query = """SELECT team,COUNT(events_shot.event_id) AS team_shot_count
FROM events_shot
JOIN events ON events_shot.event_id = events.event_id
JOIN matches ON events.match_id = matches.match_id
WHERE matches.season_id = 44 AND matches.competition_id = 2
GROUP BY events.team, events.team_id
HAVING COUNT(events.event_id) > 0
...
def Q_7(conn, execution_time):
    ...
    query = """SELECT player, COUNT(events_pass.event_id) AS through_ball_count
FROM events_pass 
JOIN events ON events_pass.event_id = events.event_id
JOIN matches ON events.match_id = matches.match_id
...
def Q_8(conn, execution_time):
    ...
    query = """SELECT team, COUNT(events_pass.event_id) AS through_ball_count
FROM events_pass 
JOIN events ON events_pass.event_id = events.event_id
JOIN matches ON events.match_id = matches.match_id
WHERE season_id = 90 AND competition_id = 11 AND technique_id = 108
...
def Q_9(conn, execution_time):
    ...
    query = """SELECT player, COUNT(events_dribble.event_id) AS completed_dribbles
FROM events_dribble
JOIN events ON events_dribble.event_id = events.event_id
JOIN matches ON events.match_id = matches.match_id
WHERE season_id in (4, 42, 90) AND competition_id = 11 AND complete = TRUE
GROUP BY player, player_id
...
def Q_10(conn, execution_time):
    ...
    query = """SELECT player, COUNT(events_dribbledpass.event_id) AS passed_dribbles
